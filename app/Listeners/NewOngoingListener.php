...
class NewOngoingListener implements ShouldQueue
{
    /**
     * Handle the event.
     */
    public function handle(ProjectEvent $event): void
    {
        try {
            ...
            Log::error('Error processing ongoing project event: ' . $exception->getMessage(), [
                'event' => $event,
    ...
    private function updateChartData(ProjectEvent $event): void
    {
        ...
        $monthlyData = $this->updateMonthlyOngoingData($chartCache, $month);
    ...
    private function updateMonthlyOngoingData(ChartYearOf $chartCache, string $month): array
    {
