<html>

    <head>
        <meta http-equiv="content-type" content="text/html; charset=utf-8">
        <title></title>
        <meta name="generator" content="LibreOffice 24.8.0.3 (Windows)">
        <meta name="created" content="2023-12-24T03:58:00">
        <meta name="changedby" content="Reanz Arthur Monera">
        <meta name="changed" content="2024-09-10T03:40:00">
        <meta name="AppVersion" content="16.0000">
        <style type="text/css">
            body {
                width: 100%;
                max-width: 794px;
                margin: 0 auto;
                line-height: 1.5;
                padding-bottom: 20px;
                font-family: Arial, sans-serif;
                font-size: 9pt;
            }

            p {
                line-height: 115%;
                text-align: left;
                orphans: 2;
                widows: 2;
                margin-bottom: 0.25cm;
                direction: ltr;
                background: transparent;
                font-size: 9pt;
            }

            table {
                width: 100%;
                border-collapse: collapse;
                page-break-inside: avoid;
                margin: 0 auto;
            }

            td,
            th {
                padding: 4px;
                font-family: Arial, sans-serif;
                font-size: 9pt;
            }

            .content-table td,
            .content-table th {
                border: 1px solid #000000;
            }

            /* Override any inline font sizes */
            font[size="2"],
            font[size="3"] {
                font-size: 9pt !important;
            }

            .tg-wrap {
                width: 100%;
                margin: 0 auto;
            }

            /* Ensure consistent font sizes for specific classes */
            .font-normal,
            .tg-cly1,
            .tg-8d8j,
            .tg-nrix,
            .tg-0lax,
            .tg-yla0 {
                font-size: 9pt;
                font-family: Arial, sans-serif;
            }

            /* Remove fixed widths from table cells */
            table[autosize="1"] td {
                width: auto;
            }

            /* Adjust form container */
            .form-container {
                width: 100%;
                margin: 0 auto;
                font-family: Arial, sans-serif;
                font-size: 9pt;
            }

            /* Consistent styling for labels and inputs */
            .label,
            .input,
            .contact-label {
                font-size: 9pt;
                font-family: Arial, sans-serif;
            }
        </style>
    </head>

    <body lang="en-PH" link="#000080" vlink="#800000" dir="ltr">

        <p align="center" style="line-height: 150%; margin-bottom: 0cm">
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 10pt">
                    <b>SETUP
                        Form 003 – Status Report (as of
                    </b>
                </font>
            </font>
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 10pt">
                    <u>
                        <b>1</b>
                    </u>
                </font>
            </font>
            <sup>
                <font face="Arial, sans-serif">
                    <font size="2" style="font-size: 10pt">
                        <u>
                            <b>st</b>
                        </u>
                    </font>
                </font>
            </sup>
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 10pt">
                    <u>
                        <b>
                            Semester 2024
                        </b>
                    </u>
                </font>
            </font>
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 10pt">
                    <b>)</b>
                </font>
            </font>
        </p>
        <p align="center" style="line-height: 150%; margin-bottom: 0cm"><br>

        </p>
        <table autosize="1" class="content-table" cellpadding="7" cellspacing="0" style=" page-break-inside: avoid;">
            <colgroup>
                <col width="18">
                <col width="146">
                <col width="4">
                <col width="513">
            </colgroup>
            <tbody>
                <tr valign="top">
                    <td width="18" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">I.</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="146" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Project
                                            Title</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="4" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="513" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            &nbsp;{{ $projectTitle }}
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="18" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="146" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="4" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="513" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="18" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">II.</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="146" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Project
                                            Cooperator</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="4" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="513" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            &nbsp;{{ $projectCooperator }}

                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="18" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">III.</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="146" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Project
                                            Duration</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="4" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="513" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            &nbsp;{{ $projectDuration }}

                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="18" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">IV.</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="146" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Amount
                                            of SETUP</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="4" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="513" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            &nbsp;{{ $setupAssistance }}
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="18" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="146" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Assistance</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="4" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="513" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="18" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">V.</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="146" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Date
                                            Funds Released</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="4" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="513" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>
                            &nbsp;{{ $fundsReleasedDate }}
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="18" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="146" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">to
                                            the Cooperator</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="4" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="513" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="18" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">VI.</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="146" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Refund
                                            Period</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="4" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="513" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            &nbsp;{{ $refundPeriod }}
                        </p>
                    </td>
                </tr>
            </tbody>
        </table>
        <p style="line-height: 100%; margin-bottom: 0cm"><br>

        </p>
        <ul>
            <li>
                <p style="line-height: 100%; margin-bottom: 0cm">
                    <font face="Arial, sans-serif">
                        <font size="2" style="font-size: 11pt">Expected
                            Output vs. Actual Accomplishment (include training and consultancy
                            service/s to be provided):</font>
                    </font>
                </p>
            </li>
        </ul>
        <p style="line-height: 100%; margin-bottom: 0cm; page-break-after: avoid;">
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 11pt">2015</font>
            </font>
        </p>
        <table autosize="1" class="content-table" cellpadding="7" cellspacing="0"
            style="page-break-inside: avoid; border-collapse: collapse;  border-bottom: 1px solid #000;">
            <colgroup>
                <col width="153">
                <col width="345">
                <col width="196">
            </colgroup>
            <tbody>
                <tr valign="top">
                    <td width="153" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Expected
                                                Output</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="345" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Actual
                                                Accomplishment</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="196" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Remarks/
                                                Justification</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                @forelse ($ExpectedAndActualData as $row)
                    <tr valign="top">
                        <td width="153" style="border-left: 1px solid #000000; border-right: 1px solid #000000; ">
                            <ul>
                                <li style="font-family: arial; font-size: 10pt">
                                    {{ $row['Expected_Output'] }}
                                </li>
                            </ul>
                        </td>
                        <td width="345" style="border-right: 1px solid #000000 ; ">
                            <ul>
                                <li style="font-family: arial; font-size: 10pt">
                                    {{ $row['Actual_Accomplishment'] }}
                                </li>
                            </ul>
                        </td>
                        <td width="196"
                            style="border-right: 1px solid #000000 ; font-family: arial; font-size: 10pt">
                            {{ $row['Remarks_Justification'] }}
                        </td>
                    </tr>
                @empty
                    <tr valign="top">
                        <td width="153" style="border-right: 1px solid #000000 ; ">
                            <ul>
                                <li>
                                </li>
                            </ul>
                        </td>
                        <td width="345" style="border-right: 1px solid #000000 ; ">
                            <ul>
                                <li>
                                </li>
                            </ul>
                        </td>
                        <td width="196" style="border-right: 1px solid #000000 ; ">
                            <p align="left" style="orphans: 2; widows: 2">
                                <br>

                            </p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        <p style="line-height: 100%; margin-bottom: 0cm"><br>
        </p>
        <p style="line-height: 100%; margin-bottom: 0cm; page-break-after: avoid;">
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 11pt">2022</font>
            </font>
        </p>
        <p style="line-height: 100%; margin-bottom: 0cm"><br>
        </p>
        <ul>
            <li>
                <p style="line-height: 100%; margin-bottom: 0cm">
                    <font face="Arial, sans-serif">
                        <font size="2" style="font-size: 11pt">List
                            of equipment/facilities purchased/fabricated with corresponding
                            cost/value:</font>
                    </font>
                </p>
            </li>
        </ul>
        <p style="line-height: 100%; margin-bottom: 0cm; page-break-after:avoid">
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 11pt">2015</font>
            </font>
        </p>
        <table width="738" class="content-table" cellpadding="7" cellspacing="0"
            style="page-break-inside: avoid; page-break-before: avoid; border-collapse: collapse;">
            <colgroup>
                <col width="24">
                <col width="115">
                <col width="82">
                <col width="28">
                <col width="112">
                <col width="82">
                <col width="88">
                <col width="93">
            </colgroup>
            <tbody>
                <tr valign="top">
                    <td colspan="3" width="249" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 9pt"><b>Approved
                                                S&amp;T Intervention Related Equipment</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td colspan="3" width="250" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 9pt"><b>Actual
                                                S&amp;T Intervention Related Equipment Acquired</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td rowspan="2" width="88" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial Narrow, sans-serif">
                                        <font size="2" style="font-size: 9pt"><b>Indicate
                                                if with Acknowledgement Receipt of Equipment</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td rowspan="2" width="93" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 9pt"><b>Remarks/
                                                Justification</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="24" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Qty.</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="115" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Particulars</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="82" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Cost</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="28" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Qty.</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="112" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Particulars</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="82" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Cost</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                @forelse ($EquipmentData as $row)
                    <tr valign="top">
                        <td width="24" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">
                                                {{ $row['Approved_qty'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="115" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">
                                                {{ $row['Approved_Particulars'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="82" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">
                                                {{ $row['Approved_cost'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="28" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">
                                                {{ $row['Actual_qty'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="112" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">
                                                {{ $row['Actual_Particulars'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="82" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">
                                                {{ $row['Actual_cost'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="88" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">
                                                {{ $row['acknowledgement'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="93" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">{{ $row['remarks'] }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                    </tr>
                @empty
                    <tr valign="top">
                        <td width="24" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="115" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="82" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="28" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="112" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="82" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="88" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="93" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        <p style="line-height: 100%; margin-bottom: 0cm"><br>

        </p>
        <ul>
            <li>
                <p style="line-height: 100%; margin-bottom: 0cm">
                    <font face="Arial, sans-serif">
                        <font size="2" style="font-size: 11pt">Non-equipment
                            items provided (packaging, etc.):</font>
                    </font>
                </p>
            </li>
        </ul>
        <table width="735" class="content-table" cellpadding="7" cellspacing="0"
            style="page-break-inside: avoid; border-collapse: collapse;">
            <colgroup>
                <col width="24">
                <col width="157">
                <col width="91">
                <col width="25">
                <col width="157">
                <col width="84">
                <col width="97">
            </colgroup>
            <tbody>
                <tr>
                    <td colspan="3" width="300" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 9pt"><b>Approved
                                                Items of Expenditure</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td colspan="3" width="295" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 9pt"><b>Actual
                                                Expenditure</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="97" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 9pt"><b>Remarks/
                                                Justification</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="24" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Qty.</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="157" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Particulars</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="91" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Cost</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="25" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Qty.</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="157" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Particulars</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="84" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Cost</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="97" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                @forelse ($NonEquipmentData as $row)
                    <tr valign="top">
                        <td width="24" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                {{ $row['Approved_qty'] ?? '' }}
                            </p>
                        </td>
                        <td width="157" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">
                                                {{ $row['Approved_Particulars'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="91" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                {{ $row['Approved_cost'] ?? '' }}

                            </p>
                        </td>
                        <td width="25" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                {{ $row['Actual_qty'] ?? '' }}

                            </p>
                        </td>
                        <td width="157" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">
                                                {{ $row['Actual_Particulars'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="84" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                {{ $row['Actual_cost'] ?? '' }}
                            </p>
                        </td>
                        <td width="97" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                {{ $row['remarks'] ?? '' }}
                            </p>
                        </td>
                    </tr>
                @empty
                    <tr valign="top">
                        <td width="24" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">

                            </p>
                        </td>
                        <td width="157" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="91" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                            </p>
                        </td>
                        <td width="25" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                            </p>
                        </td>
                        <td width="157" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="84" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                            </p>
                        </td>
                        <td width="97" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                            </p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        <p style="line-height: 100%; margin-bottom: 0cm"><br>

        </p>
        <ul>
            <li>
                <p style="line-height: 100%; margin-bottom: 0cm">
                    <font face="Arial, sans-serif">
                        <font size="2" style="font-size: 11pt">Status
                            of Fund Utilization:</font>
                    </font>
                </p>
            </li>
        </ul>
        <p style="line-height: 100%; margin-bottom: 0cm">
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 11pt">2015</font>
            </font>
        </p>
        <table width="736" class="content-table" cellpadding="7" cellspacing="0" style=" page-break-inside: avoid;">
            <colgroup>
                <col width="401">
                <col width="5">
                <col width="288">
            </colgroup>
            <tbody>
                <tr valign="top">
                    <td width="401" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Total
                                            Approved Project Cost</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="5" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="288" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            ₱ <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">
                                            {{ $total_approved_project_cost }}</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="401" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Amount
                                            Utilized per Financial Report (as of </font>
                                    </font>
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt"><u>June
                                                2024</u></font>
                                    </font>
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">)</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="5" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="288" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            ₱ <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">{{ $amount_utilized }}</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="401" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Remarks
                                            on Status of Utilization</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="5" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="288" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">
                                            {{ $remarks_on_status_of_utilization }}</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
            </tbody>
        </table>
        <p style="line-height: 100%; margin-bottom: 0cm"><br>

        </p>
        <ul>
            <li>
                <p style="line-height: 100%; margin-bottom: 0cm">
                    <font face="Arial, sans-serif">
                        <font size="2" style="font-size: 11pt">Status
                            of Refund:</font>
                    </font>
                </p>
            </li>
        </ul>
        <p style="line-height: 100%; margin-bottom: 0cm; page-break-after: avoid;">
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 11pt">2015</font>
            </font>
        </p>
        <table width="714" class="content-table" cellpadding="7" cellspacing="0" style=" page-break-inside: avoid;">
            <colgroup>
                <col width="335">
                <col width="10">
                <col width="214">
                <col width="99">
            </colgroup>
            <tbody>
                <tr valign="top">
                    <td width="335" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Total
                                            amount to be refunded</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="10" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="214" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            ₱ <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">
                                            {{ $total_amount_to_be_refunded }}</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="99" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="335" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Approved
                                            refund schedule</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="10" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td colspan="2" width="327" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">
                                            {{ $approved_refund_schedule }}</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="335" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Total
                                            amount already due (as of </font>
                                    </font>
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt"><u>June
                                                2024</u></font>
                                    </font>
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">)</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="10" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="214" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">
                                            {{ $total_amount_already_due }}</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="99" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="335" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Total
                                            amount refunded</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="10" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="214" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            ₱ <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">{{ $total_amount_refunded }}
                                        </font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="99" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="335" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Unsettled
                                            refund</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="10" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="214" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">{{ $unsetted_refund }}</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="99" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="335" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Refund
                                            delayed since</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="10" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td colspan="2" width="327" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">{{ $refund_delayed_since }}
                                        </font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
            </tbody>
        </table>
        <p style="line-height: 100%; margin-bottom: 0cm"><br>

        </p>
        <ul>
            <li>
                <p style="line-height: 100%; margin-bottom: 0cm">
                    <font face="Arial, sans-serif">
                        <font size="2" style="font-size: 11pt">Volume
                            and value of production including sales generated:</font>
                    </font>
                </p>
            </li>
        </ul>
        <table width="735" class="content-table" cellpadding="7" cellspacing="0"
            style=" page-break-inside: avoid; border-collapse: collapse;">
            <colgroup>
                <col width="237">
                <col width="148">
                <col width="142">
                <col width="151">
            </colgroup>
            <tbody>
                <tr valign="top">
                    <td width="237" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Name
                                                of Product/ Service</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="148" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Volume
                                                of Production</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="142" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Quarter
                                                (specify)</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="151" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>Gross
                                                Sales</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                @forelse ($SalesData as $row)
                    <tr valign="top">
                        <td width="237" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">
                                                {{ $row['ProductService'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="148" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                {{ $row['SalesVolumeProduction'] }}

                            </p>
                        </td>
                        <td width="142" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">3</font>
                                        </font><sup>
                                            <font face="Arial, sans-serif">
                                                <font size="2" style="font-size: 10pt">rd</font>
                                            </font>
                                        </sup>
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">
                                                {{ $row['SalesQuarter'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="151" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                ₱ <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">
                                                {{ $row['GrossSales'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                    </tr>
                @empty
                    <tr valign="top">
                        <td width="237" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="148" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2"></p>
                        </td>
                        <td width="142" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt">3</font>
                                        </font><sup>
                                            <font face="Arial, sans-serif">
                                                <font size="2" style="font-size: 10pt">rd</font>
                                            </font>
                                        </sup>
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="151" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                ₱ <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 10pt"></font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                    </tr>
                @endforelse
                <tr>
                    <td colspan="3" width="555" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt"><b>Total</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="151" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="right" style="orphans: 2; widows: 2">
                            ₱ <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>3,573,500.00</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
            </tbody>
        </table>
        <p style="line-height: 100%; margin-bottom: 0cm"><br>
        </p>
        <ul>
            <li>
                <p style="line-height: 100%; margin-bottom: 0cm">
                    <font face="Arial, sans-serif">
                        <font size="2" style="font-size: 11pt">No.
                            of new employment generated from the project:</font>
                    </font>
                </p>
            </li>
        </ul>
        <p style="line-height: 100%; margin-bottom: 0cm">
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 11pt">1</font>
            </font><sup>
                <font face="Arial, sans-serif">
                    <font size="2" style="font-size: 11pt">st</font>
                </font>
            </sup>
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 11pt">
                    Quarter</font>
            </font>
        </p>
        <table width="738" class="content-table" cellpadding="7" cellspacing="0"
            style=" page-break-inside: avoid; border-collapse: collapse;">
            <colgroup>
                <col width="169">
                <col width="159">
                <col width="159">
                <col width="193">
            </colgroup>
            <tbody>
                <tr valign="top">
                    <td width="169" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>No.
                                                of Employees</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="159" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>No.
                                                of Male</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="159" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>No.
                                                of Female</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="193" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt"><b>No.
                                                of Person with Disability</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                @forelse ($EmploymentGeneratedData as $row)
                    <tr valign="top">
                        <td width="169" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">
                                                {{ $row['Employment_total'] ?? 0 }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="159" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">
                                                {{ $row['Employment_Male'] ?? 0 }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="159" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">
                                                {{ $row['Employment_Female'] ?? 0 }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="193" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">
                                                {{ $row['Employment_PWD'] ?? 0 }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                    </tr>
                @empty
                    <tr valign="top">
                        <td width="169" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">0</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="159" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">0</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="159" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">0</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="193" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">0</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        <p style="line-height: 100%; margin-bottom: 0cm"><br>

        </p>
        <ul>
            <li>
                <p style="line-height: 100%; margin-bottom: 0cm">
                    <font face="Arial, sans-serif">
                        <font size="2" style="font-size: 11pt">No.
                            of new indirect employment from the project:</font>
                    </font>
                </p>
            </li>
        </ul>
        <table width="735" class="content-table" cellpadding="7" cellspacing="0"
            style=" page-break-inside: avoid; border-collapse: collapse;">
            <colgroup>
                <col width="345">
                <col width="52">
                <col width="52">
                <col width="46">
                <col width="49">
                <col width="49">
                <col width="42">
            </colgroup>
            <tbody>
                <tr valign="top">
                    <td width="345" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">No.
                                            of Indirect Employment</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td colspan="3" width="178" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Forward</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td colspan="3" width="169" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Backward</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="345" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="52" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Male</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="52" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Female</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="46" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Total</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="49" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Male</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="49" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Female</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="42" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Total</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                @forelse ($IndirectEmploymentData as $row)
                    <tr>
                        <td width="345" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">
                                                {{ $row['IndirectEmployment_total'] ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="52" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <br>
                                {{ $row['IndirectEmployment_ForwardMale'] ?? 0 }}

                            </p>
                        </td>
                        <td width="52" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <br>
                                {{ $row['IndirectEmployment_ForwardFemale'] ?? 0 }}

                            </p>
                        </td>
                        <td width="46" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <br>
                                {{ $row['InderectEmplyment_ForwardTotal'] ?? 0 }}

                            </p>
                        </td>
                        <td width="49" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <br>
                                {{ $row['IndirectEmployment_BackwardMale'] ?? 0 }}

                            </p>
                        </td>
                        <td width="49" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <br>
                                {{ $row['IndirectEmployment_BackwardFemale'] ?? 0 }}

                            </p>
                        </td>
                        <td width="42" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <br>
                                {{ $row['IndirectEmployment_BackwardTotal'] ?? 0 }}

                            </p>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td width="345" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="left" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt"></font>
                                </font>
                            </p>
                        </td>
                        <td width="52" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <br>

                            </p>
                        </td>
                        <td width="52" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <br>

                            </p>
                        </td>
                        <td width="46" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <br>

                            </p>
                        </td>
                        <td width="49" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <br>

                            </p>
                        </td>
                        <td width="49" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <br>

                            </p>
                        </td>
                        <td width="42" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                            <p align="center" style="orphans: 2; widows: 2">
                                <br>

                            </p>
                        </td>
                    </tr>
                @endforelse
                <tr>
                    <td width="345" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt"><b>Total</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="52" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="52" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="46" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="49" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="49" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="42" valign="top" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
            </tbody>
        </table>
        <p style="line-height: 100%; margin-bottom: 0cm"><br>

        </p>
        <ul>
            <li>
                <p style="line-height: 100%; margin-bottom: 0cm">
                    <font face="Arial, sans-serif">
                        <font size="2" style="font-size: 11pt">List
                            of market penetrated:</font>
                    </font>
                </p>
            </li>
        </ul>
        <table width="735" class="content-table" cellpadding="7" cellspacing="0"
            style=" page-break-inside: avoid; border-collapse: collapse;">
            <colgroup>
                <col width="255">
                <col width="274">
                <col width="163">
            </colgroup>
            <tbody>
                <tr valign="top">
                    <td width="255" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt"><b>Existing
                                                Market</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td colspan="2" width="451" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt"><b>New
                                                Market</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td rowspan="2" width="255" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 10pt">
                                            {{ $existing_market }}</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="274" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt"><b>Specify
                                                Place</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="163" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="center" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt"><b>Effective
                                                Date</b></font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="274" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>
                            {{ $new_market_specific_place ?? '' }}

                        </p>
                    </td>
                    <td width="163" style="border: 1px solid #000000; padding: 0cm 0.19cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>
                            {{ $new_market_effective_date ?? '' }}

                        </p>
                    </td>
                </tr>
            </tbody>
        </table>
        <p style="line-height: 100%; margin-bottom: 0cm"><br>

        </p>
        <table width="697" class="content-table" cellpadding="7" cellspacing="0" style=" page-break-inside: avoid;">
            <colgroup>
                <col width="28">
                <col width="641">
            </colgroup>
            <tbody>
                <tr valign="top">
                    <td width="28" style="border: none; padding: 0cm">
                        <ul>
                            <li>
                                <p align="left" style="orphans: 2; widows: 2">
                                    <font face="Aptos, sans-serif">
                                        <font size="3" style="font-size: 12pt">
                                            <font face="Arial, sans-serif">
                                                <font size="2" style="font-size: 11pt">VII.</font>
                                            </font>
                                        </font>
                                    </font>
                                </p>
                            </li>
                        </ul>
                    </td>
                    <td width="641" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Improvement
                                            in production efficiency (includes quantitative indicators on
                                            improvement in number</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="28" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="641" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">and
                                            quality of materials, number and value of produce, waste
                                            minimization, reject reduction, etc.)</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr>
                    <td colspan="2" width="683" valign="top" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                @forelse ($improvement_in_production as $improvement)
                    <tr valign="top">
                        <td width="28" style="border: none; padding: 0cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">-</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="641" style="border: none; padding: 0cm">
                            <p align="justify" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">{{ $improvement ?? '' }}
                                            </font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                    </tr>
                @empty
                    <tr valign="top">
                        <td width="28" style="border: none; padding: 0cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">-</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="641" style="border: none; padding: 0cm">
                            <p align="justify" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">none</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                    </tr>
                @endforelse
                <tr>
                    <td colspan="2" width="683" valign="top" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="28" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">VIII.</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="641" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Problems
                                            met &amp; actions taken during project implementation</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr>
                    <td colspan="2" width="683" valign="top" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                @forelse ($problems_meet as $problem)
                    <tr valign="top">
                        <td width="28" style="border: none; padding: 0cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">-</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="641" style="border: none; padding: 0cm">
                            <p align="justify" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">{{ $problem ?? '' }}
                                            </font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                    </tr>
                @empty
                    <tr valign="top">
                        <td width="28" style="border: none; padding: 0cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">-</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="641" style="border: none; padding: 0cm">
                            <p align="justify" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">none</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                    </tr>
                @endforelse
                <tr>
                    <td colspan="2" width="683" valign="top" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="28" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">IX.</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="641" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Action/
                                            plan for the improvement of project’s operation</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr>
                    <td colspan="2" width="683" valign="top" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                @forelse ($action_and_plan as $action)
                    <tr valign="top">
                        <td width="28" style="border: none; padding: 0cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">-</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="641" style="border: none; padding: 0cm">
                            <p align="justify" style="orphans: 2; widows: 2; margin-bottom: 0cm">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">{{ $action ?? '' }}</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                            <p align="justify" style="orphans: 2; widows: 2"><br>

                            </p>
                        </td>
                    </tr>
                @empty
                    <tr valign="top">
                        <td width="28" style="border: none; padding: 0cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">-</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="641" style="border: none; padding: 0cm">
                            <p align="justify" style="orphans: 2; widows: 2; margin-bottom: 0cm">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">none</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                            <p align="justify" style="orphans: 2; widows: 2"><br>

                            </p>
                        </td>
                    </tr>
                @endforelse
                <tr>
                    <td width="28" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">X.</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="641" valign="top" style="border: none; padding: 0cm">
                        <p align="justify" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Linkages/Promotional
                                            Plan</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr>
                    <td colspan="2" width="683" valign="top" style="border: none; padding: 0cm">
                        <p align="justify" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                @forelse ($linkages_promotional_plan as $linkage)
                    <tr valign="top">
                        <td width="28" style="border: none; padding: 0cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">-</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="641" style="border: none; padding: 0cm">
                            <p align="justify" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">{{ $linkage ?? '' }}
                                            </font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                    </tr>
                @empty
                    <tr valign="top">
                        <td width="28" style="border: none; padding: 0cm">
                            <p align="right" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">-</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                        <td width="641" style="border: none; padding: 0cm">
                            <p align="justify" style="orphans: 2; widows: 2">
                                <font face="Aptos, sans-serif">
                                    <font size="3" style="font-size: 12pt">
                                        <font face="Arial, sans-serif">
                                            <font size="2" style="font-size: 11pt">none</font>
                                        </font>
                                    </font>
                                </font>
                            </p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        <p style="line-height: 100%; margin-bottom: 0cm"><br>

        </p>
        <p style="margin-bottom: 0.28cm">
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 11pt"> </font>
            </font>
        </p>
        <p style="margin-bottom: 0.28cm">
            <font face="Arial, sans-serif">
                <font size="2" style="font-size: 11pt"> </font>
            </font>
        </p>
        <table width="751" class="content-table" cellpadding="7" cellspacing="0" style=" page-break-inside: avoid;">
            <colgroup>
                <col width="209">
                <col width="16">
                <col width="190">
                <col width="16">
                <col width="250">
            </colgroup>
            <tbody>
                <tr valign="top">
                    <td width="209" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2; margin-bottom: 0cm">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Prepared
                                            by:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="16" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="190" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Reviewed
                                            and Endorsed by:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="16" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="250" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Noted
                                            by:</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="209"
                        style="border-top: none; border-bottom: 1px solid #000000; border-left: none; border-right: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="16" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="190"
                        style="border-top: none; border-bottom: 1px solid #000000; border-left: none; border-right: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="16" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="250"
                        style="border-top: none; border-bottom: 1px solid #000000; border-left: none; border-right: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <td width="209" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">PSTD</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="16" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="190" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">RPMO
                                            Manager</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                    <td width="16" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <br>

                        </p>
                    </td>
                    <td width="250" style="border: none; padding: 0cm">
                        <p align="left" style="orphans: 2; widows: 2">
                            <font face="Aptos, sans-serif">
                                <font size="3" style="font-size: 12pt">
                                    <font face="Arial, sans-serif">
                                        <font size="2" style="font-size: 11pt">Regional
                                            Director</font>
                                    </font>
                                </font>
                            </font>
                        </p>
                    </td>
                </tr>
            </tbody>
        </table>
        <p style="margin-bottom: 0.28cm"><br>
            <br>

        </p>
        <div title="footer">
            <p
                style="line-height: 100%; margin-top: 0.66cm; margin-bottom: 0cm; border-top: none; border-bottom: 1.50pt solid #000000; border-left: none; border-right: none; padding-top: 0cm; padding-bottom: 0.04cm; padding-left: 0cm; padding-right: 0cm">
                <br>

            </p>

        </div>
        <htmlpagefooter name="page-footer">
            <p style="line-height: 100%; margin-bottom: 0cm">
                <font face="Arial Narrow, sans-serif">
                    <font size="2" style="font-size: 10pt">SETUP
                        Guidelines (Revision 3.0) Annex A-3: SETUP Form 003 – Status
                        Report Page 3 of 3</font>
                </font>
            </p>
        </htmlpagefooter>
    </body>

</html>
