...
function RemoveRowHandler(ButtonSelector: string, ContainerSelectors: string) {
    ...
    $(document).off('click', ButtonSelector).on('click', ButtonSelector, function() {
        ...
        if (rows.length > 1) {
            ...
            console.warn('Cannot remove the last remaining row in', ContainerSelectors);
        }
    });

}

const toggleDeleteRowButton = (container: JQuery<Element>, elementSelector: string) => {
